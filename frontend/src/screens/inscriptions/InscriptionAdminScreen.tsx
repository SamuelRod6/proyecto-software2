...
const statusFilterOptions = [{ value: "", label: "Todos" }, ...statusOptions];
...
export default function InscriptionAdminScreen(): JSX.Element {
    ...
    const eventFilterOptions = useMemo(() => {
      return [{ value: "", label: "Todos" }, ...eventOptions];
    }, [eventOptions]);
      ...
      <section className="space-y-6 bg-slate-900 min-h-screen px-4 py-8">
        <header>
          ...
          <p className="text-slate-300">
            Revisa y actualiza el estado de los usuarios inscritos.
          </p>
        </header>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.1fr_1.1fr_1.4fr]">
          ...
          <SelectInput
            ...
            value={eventFilter}
            onChange={(value) =>
              setEventFilter(Array.isArray(value) ? (value[0] ?? "") : value)
            }
          ...
          {selected && (
            <div className="space-y-4">
              <div className="rounded-lg border border-slate-700 bg-slate-900/60 p-4 text-sm text-slate-200">
                ...
                <p className="text-slate-400">
                  Fecha inscripción: {selected.fecha_inscripcion}
                </p>
              </div>
              <SelectInput
                value={newStatus}
                onChange={(value) =>
                  setNewStatus(Array.isArray(value) ? (value[0] ?? "") : value)
                }
