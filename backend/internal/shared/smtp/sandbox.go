...
const defaultSandboxURL = "https://sandbox.api.mailtrap.io/api/send/4448626"
...
func SendSandboxEmail(ctx context.Context, input SandboxSendRequest) (*SandboxSendResponse, error) {
	url := strings.TrimSpace(os.Getenv("MAILTRAP_SANDBOX_URL"))
	if url == "" {
		url = defaultSandboxURL
	}
	token := strings.TrimSpace(os.Getenv("MAILTRAP_SANDBOX_TOKEN"))
	if token == "" {
		return nil, fmt.Errorf("MAILTRAP_SANDBOX_TOKEN is not set")
	}
